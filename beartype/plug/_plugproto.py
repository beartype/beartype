...
if IS_PYTHON_AT_LEAST_3_8:
    ...
    #get that class right. It's a ball-and-chain dragging @beartype down.)
    #
...
# this case, fallback to defining classes with the same names but instead
# implemented as standard abstract base classes (ABCs).
else:
    # ....................{ IMPORTS                        }....................
    # Defer version-specific imports.
    from abc import (
    ...
    # ....................{ SUBCLASSES                     }....................
    class BeartypeHintable(metaclass=ABCMeta):  # type: ignore[no-redef]
        @classmethod
        @abstractmethod
        def __beartype_hint__(cls) -> object:
            pass
...
# ....................{ PROTOCOLS ~ docs                   }....................
# Document this protocol regardless of implementation.
BeartypeHintable.__doc__ = (
    '''
    ...
    *any* user-defined class defining a :mod:`beartype`-specific
    ...
    explicitly subclass this protocol. User-defined classes leveraging a custom
    metaclass incompatible with the standard :class:`abc.ABCMeta` metaclass are
    incapable of subclassing this protocol and thus exempt.
    ''')


# Note that we necessarily define the docstring on the unbound function wrapped
# by the method descriptor rather than on the method descriptor itself. Why?
# Because CPython insanely prohibits setting the docstring directly on the
# method descriptor itself. Attempting to do so raises the following exception:
#     AttributeError: attribute '__doc__' of 'method' objects is not writable
BeartypeHintable.__beartype_hint__.__func__.__doc__ = (  # type: ignore[attr-defined]
    '''
