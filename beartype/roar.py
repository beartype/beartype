...
class BeartypeDecorHintPepInvalidException(BeartypeDecorHintPepException):
    '''
    **Beartype decorator invalid PEP-compliant type hint exception.**
...
class BeartypeDecorHintPepParamException(BeartypeDecorHintPepException):
    '''
    **Beartype decorator parameter PEP-compliant type hint exception.**

    This exception is raised at decoration time from the
    :func:`beartype.beartype` decorator on receiving a callable declaring
    parameters annotated by invalid PEP-compliant type hints, including those
    annotated by:

    * The :attr:`typing.NoReturn` type hint (e.g.,
      ``def bad_func(bad_param: NoReturn): pass``).
    '''
