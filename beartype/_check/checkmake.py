...
#beartype._decor.wrap._wrapcode.make_func_pith_code() factory in terms of
...
#    make_func_pith_code() factory. Indeed, it seems likely that either:
#    * make_func_pith_code() should internally call make_func_raiser_code().
#      This is *PROBABLY* the right approach, but research is warranted.
#    * make_func_raiser_code() should internally call make_func_pith_code().
...
from beartype._check._checksnip import (
    ...
    CODE_VIOLATION_GET_CLS_STACK,
    CODE_VIOLATION_GET_RANDOM_INT,
    CODE_VIOLATION_RAISE,
    CODE_VIOLATION_WARN,
)
...
from beartype._util.text.utiltextjoin import join_delimited_conjunction
from itertools import count
from warnings import warn

# ....................{ FACTORIES ~ code : check           }....................
...
#FIXME: Unit test us up, please.
@callable_cached
def make_code_raiser_check(
    # Mandatory parameters.
    hint: object,
    conf: BeartypeConf,
    code_template_get_violation: str,

    # Optional parameters.
    cls_stack: Optional[TypeStack] = None,
    ...
    code_template_get_violation : str
        ...
        when an arbitrary object violates this hint under this configuration.
    ...
    :func:`.make_check_expr`
        Further details.
    '''
    assert isinstance(code_template_get_violation, str), (
        f'{repr(code_template_get_violation)} not string.')
    ...
    arg_cls_stack = CODE_VIOLATION_GET_CLS_STACK if cls_stack else ''
    ...
    code_get_violation = code_template_get_violation.format(
    ...
    # non-fatal warning.
    code_handle_violation = make_code_raiser_violation(
        conf=conf, func_scope=func_scope, is_param=is_param)

    # Code snippet type-checking the root pith against the root hint.
    func_code = (
        f'{CODE_RAISER_CHECK_PREFIX}'
        f'{code_expr}'
...
# ....................{ FACTORIES ~ code : violation       }....................
def make_code_raiser_violation(
    # Mandatory parameters.
    conf: BeartypeConf,
    ...
    if (
        ...
        code_violation = CODE_VIOLATION_WARN
    ...
    # Else...
        ...
        code_violation = CODE_VIOLATION_RAISE
...
def _make_func_checker(
    ...
    **This factory intentionally accepts no** ``exception_cls`` **parameter.**
    ...
    if hint_refs_type_basename:
        # Double-quoted and comma-delimited conjunction (e.g., "..., and ...")
        # of these relative forward references.
        hint_forwardrefs_conjunction = join_delimited_conjunction(
            strs=hint_refs_type_basename, is_double_quoted=True)

        # Raise an exception embedding this conjunction.
        raise BeartypeDecorHintForwardRefException(
            ...
            f'{hint_forwardrefs_conjunction}. '
            f'Beartype currently prohibits relative forward references '
            f'outside @beartype-decorated callables. '
            f'For your own personal safety and '
            ...
            f'(e.g., by replacing "MuhClass" with '
            f'"muh_package.muh_module.MuhClass").'
        )
    # Else, this hint contains *NO* relative forward references.

    # Unqualified basename of this type-checking function, uniquified by
    # suffixing an arbitrary integer guaranteed to be unique to this function.
    func_checker_name = (
        f'{FUNC_CHECKER_NAME_PREFIX}{next(_func_checker_name_counter)}')

    # Python code snippet declaring the signature of the type-checking function
    # function to be defined and returned by this factory.
    ...
    func_tester = make_func(
        ...
        func_label=f'{EXCEPTION_PLACEHOLDER}type-checker {func_checker_name}()',
