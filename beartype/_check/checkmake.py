...
CodeRaiserFuncCheck = tuple[str, LexicalScope, TupleBeartypeForwardRefs]
'''
...
low-level private code factories defined by this submodule as well as metadata
describing that code).

This hint matches a 3-tuple ``(func_wrapper_code, func_wrapper_locals,
beartype_ref_proxies)``, where:
...
* ``beartype_ref_proxies`` is the proper subset of the items of
  ``func_wrapper_locals`` that are :mod:`beartype`-specific **forward reference
  proxies** (i.e., classes whose metaclass defers the resolution of forward
  reference type hints referencing type hints that have yet to be defined in the
  lexical scopes of external callers).
'''
    ...
    **This factory dynamically resolves all** :pep:`484`-compliant **forward
    ...
    try:
        ...
        with catch_warnings(record=True) as warnings_issued:
            ...
            #FIXME: In this case, deal with
            #"func_scope_frozen.beartype_ref_proxies", please. *sigh*
