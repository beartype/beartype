...
type hint factories** (i.e., subscriptable by exactly two child type hints)
originating from an **isinstanceable origin type** (i.e., isinstanceable class
such that *all* objects satisfying this hint are instances of this class).

Note that the corresponding types in the typing module all define a private
``_nparams`` instance variable equal to 1.
'''
...
Note that the corresponding types in the typing module all define a private
``_nparams`` instance variable equal to 2.
'''


# Fully initialized and coerced to a "frozenset" by the _init() function below.
HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_3: _FrozenSetHintSign = {  # type: ignore[assignment]
    HintSignCoroutine,
    HintSignGenerator,
}
'''
...
Note that the corresponding types in the typing module all define a private
``_nparams`` instance variable equal to 3.
'''
...
def _init() -> None:
    ...
    # Allow these global variables to be redefined by this function.
    global \
    ...
    if IS_PYTHON_AT_LEAST_3_13:
        ...
        # under Python >= 3.13, which generalized various single-argument type
        # hint factories to accept an additional optional child type hint via
        # "PEP 696 – Type Defaults for Type Parameters".
        HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_2.update((  # type: ignore[attr-defined]
            HintSignAsyncContextManager,
            HintSignContextManager,
        ))
    ...
    # case...
        ...
        # Add all signs uniquely identifying two-argument type hint factories
        # under Python <= 3.12.
        HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_1.update((  # type: ignore[attr-defined]
            HintSignAsyncContextManager,
            HintSignContextManager,
        ))

    # Coerce these previously mutable sets into immutable frozen sets.
    ...
    HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_3 = frozenset(
        HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_3)
    # print(f'HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_1: {HINT_SIGNS_ORIGIN_ISINSTANCEABLE_ARGS_1}')
