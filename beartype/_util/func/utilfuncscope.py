...
def get_func_locals(
    ...
    **This high-level getter is inefficient and should thus only be called if
    ...
    if (
        ...
        # This callable is nested but the active Python interpreter fails to
        # declare the sys._getframe() getter...
        ...
        return {}
    # Else, this callable is nested *AND* the active Python interpreter
    # declares the sys._getframe() getter.
    ...
    #   embodying callables defined by different modules) but
    ...
    while func_frame:
        ...
        if func_frame_codeobj is not None:
            ...
            # because we have exhaustively tested this with *ALL* edge cases.
                ...
                # parent callable, the local scope of this nested callable is
    ...
    # Since the above iteration exhausted itself rather than returning, that
    # iteration failed to find the frame embodying the parent callable
...
def add_func_scope_attr(
    ...
    dictionary the name to value of each globally or locally scoped attribute
    externally accessed elsewhere, whose key is a machine-readable name
    internally generated by this getter to uniquely refer to the passed object
    and whose value is that object) to the passed scope *and* return that name.

    Parameters
    ----------
    attr : Any
        Arbitrary object to be added to this scope.
    attr_scope : Dict[str, Any]
    ...
    ----------
    str
        Name of the passed object in this scope as generated by this getter.
    ...
    # Name of the new attribute referring to this object in this scope.
