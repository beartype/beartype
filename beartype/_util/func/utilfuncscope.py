...
# ....................{ ADDERS ~ type                     }....................
#FIXME: Most of the following adders and expressers import from
#"beartype._decor" and are thus unsuitable for this "beartype._util.func"
#submodule. Instead, shift these functions into a new
#"beartype._decor._code._pep._pepscope" submodule where they properly belong.
def add_func_scope_type(
    ...
    # Optional parameters.
    attr_label: str = 'Globally or locally scoped class',
) -> str:
    '''
    ...
    attr_label : str, optional
        Human-readable substring describing this class in exception messages.
        Defaults to a reasonably sane string.
    ...
    #FIXME: Shift these into global scope once we move to "_pepscope".
    # Avoid circular import dependencies.
    ...
    return (
        ...
        add_func_scope_attr(
            attr=cls, func_scope=func_scope, attr_label=attr_label)
    )
...
def add_func_scope_types(
    ...
    # Optional parameters.
    attr_label: str = 'Globally or locally scoped classes',
    ...
    * This tuple's hash. Note that this tuple's object ID is intentionally
    ...
    if not isinstance(types, _SET_OR_TUPLE):
        raise BeartypeDecorHintNonPepException(
            f'{attr_label} {repr(types)} neither set nor tuple.')
    # Else, this object is either a set or tuple.
    #
    # If this collection is empty, raise an exception.
    elif not types:
        raise BeartypeDecorHintNonPepException(f'{attr_label} empty.')
    ...
    assert isinstance(types, tuple), f'{attr_label} {repr(types)} not tuple.'
    ...
    return add_func_scope_attr(
        attr=types, func_scope=func_scope, attr_label=attr_label)
...
def express_func_scope_type_forwardref(
    # Mandatory parameters.
    #
    # Note that this attribute is intentionally *NOT* typed as a
    # "HINT_PEP484585_FORWARDREF_UNION" here, as doing so would require
    # importing that union into global scope, inviting circular imports.
    forwardref: object,
    forwardrefs_class_basename: Optional[set],
    func_scope: CallableScope,

    # Optional parameters.
    attr_label: str = 'Globally or locally scoped forward reference',
    ...
    # Else, this classname is unqualified. In this case...
        ...
        # If the set of unqualified classnames referred to by all relative
