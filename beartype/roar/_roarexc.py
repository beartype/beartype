...
class BeartypeCallHintViolation(BeartypeCallHintException):
    ...
    _culprit : object
        ...
        * If the **culprit** (i.e., user-defined object responsible for this
          ...
          to that culprit.
        * If that culprit is ``None``, the
          :data:`_BEARTYPE_CALL_HINT_VIOLATION_CULPRIT_NONE` placeholder. Since
          the :class:`weakref.ref` class ambiguously returns ``None`` when that
          culprit has already been garbage-collected, this attribute
          intentionally substitutes ``None`` for this non-``None`` placeholder.
        * If that culprit *cannot* be weakly referenced (e.g., due to being an
          instance of a builtin variable-sized C-based type), the
          machine-readable string representation of that culprit.
    ...
    # def __init__(self, message: str, culprit: object) -> None:
        ...
        culprit : object
            **Culprit** (i.e., user-defined object directly responsible for this
            ...
            parameter passed to *or* object returned from the wrapper function
            generated by the :func:`beartype.beartype` decorator raising this
            exception). For safety, this exception internally preserves only a
