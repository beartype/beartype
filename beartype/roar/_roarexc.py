...
class BeartypeCallHintViolation(BeartypeCallHintException):
    ...
    ----------
    _culprit_repr : str
        Machine-readable string representation of the object weakly referred to
        by the :attr:`_culprit_weakref` reference.
    _culprit_weakref : object
        Either:

        * If the **culprit** (i.e., user-defined object responsible for this
          exception, typically due to violating a type hint annotating a
          parameter passed to *or* object returned from the wrapper function
          generated by the :func:`beartype.beartype` decorator raising this
          exception) is *not* ``None`` and that culprit can be weakly
          referenced, a **weak reference** (i.e., :class:`weakref.ref` instance)
          to that culprit.
        * If that culprit is ``None``, a singleton non-``None`` placeholder.
          Since the :class:`weakref.ref` class ambiguously returns ``None`` when
          that culprit has already been garbage-collected, this attribute
          intentionally substitutes ``None`` for this placeholder.
        * If that culprit *cannot* be weakly referenced (e.g., due to being an
          instance of a builtin variable-sized C-based type), ``None``.

        See also the :meth:`culprit` property wrapping this private attribute.
    '''

    # ..................{ INITIALIZERS                       }..................
    #FIXME: Refactor code elsewhere to unconditionally pass "culprit", please.
    #Pick up on Monday at the "beartype._decor._error._errorsleuth" submodule!
    ...
    # def __init__(self, message: str, culprit: object) -> None:
        ...
        culprit : object
            **Culprit** (i.e., user-defined object directly responsible for this
            exception, typically due to violating a type hint annotating a
            parameter passed to *or* object returned from the wrapper function
            generated by the :func:`beartype.beartype` decorator raising this
            exception). This exception internally preserves a weak reference to
            this culprit, which callers may then safely retrieve at any time via
            the :meth:`culprit` property.
        '''
    ...
    @property
    def culprit(self) -> object:
        '''
        **Culprit** (i.e., user-defined object directly responsible for this
        exception, typically due to violating a type hint annotating a parameter
        passed to *or* object returned from the wrapper function generated by
        the :func:`beartype.beartype` decorator raising this exception).

        Caveats
        ----------
        **This property is safely accessible only from the** ``except ...:``
        **block directly catching this exception.** To avoid memory leaks, this
        property only weakly refers to this culprit and is thus usable only
        wherever this culprit is usable. Notably, this property is guaranteed to
        safely refer to this culprit *only* for the duration of the ``except
        ...:`` block directly catching this exception. Since this culprit may be
        garbage-collected at any time thereafter, this property *cannot* be
        guaranteed to safely refer to this culprit outside of that block. If
        this property is accessed from *any* other context and this culprit has
        already been garbage-collected, this property raises a
        :exc:`BeartypeCallHintViolationCulpritDead` exception.
        ...
        **This property returns the machine-readable representation of instances
        ...
        Raises
        ----------
        BeartypeCallHintViolationCulpritDead
        ...
        culprit = get_weakref_obj_or_repr(
