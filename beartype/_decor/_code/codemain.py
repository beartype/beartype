...
# ....................{ CODERS                            }....................
def generate_code(data: BeartypeData) -> Tuple[str, CallableScope]:
    '''
    ...
    ----------
    (str, CallableScope)
        2-tuple ``(func_wrapper_code, func_wrapper_locals)`` where:
        ...
        * ``func_wrapper_code`` is either:
          ...
          * If the decorated callable requires *no* type-checking (e.g., due to
            ...
            noop for unannotated callables. By compare, this boolean is
            ``True`` only for callables annotated with **ignorable type hints**
            (i.e., :class:`object`, :class:`beartype.cave.AnyType`,
          ...
          * Else, Python code defining the wrapper function
            ...
            * A signature declaring this wrapper, accepting both
              ...
              * A private ``__beartype_func`` parameter initialized to the
                decorated callable. In theory, this callable should be
                accessible as a closure-style local in this wrapper. For
                unknown reasons (presumably, a subtle bug in the exec()
                builtin), this is *not* the case. Instead, a closure-style
                local must be simulated by passing this callable at function
                definition time as the default value of an arbitrary parameter.
                To ensure this default is *not* overwritten by a function
                accepting a parameter of the same name, this unlikely edge case
        ...
        * ``func_wrapper_locals`` is all **local scope** (i.e., dictionary
          mapping from the name to value of each locally scoped attribute
          internally referenced in the signature of this wrapper function)
          required to declare this wrapper function.
    ...
    #* Refactor _code_check_params() to instead return a 2-tuple
    #  "(func_params_code, func_params_locals)".
    #* Refactor _code_check_return() to instead return a 2-tuple
    #  "(func_return_code, func_return_locals)".
    #* The "func_params_locals" and "func_return_locals" dictionaries returned
    #  by those functions should be naively merged into the local
    #  "func_wrapper_locals" dictionary declared by this function below. Note
    #  that we intentionally write "naively". In particular, attempting to
    #  detect key collisions between these three dictionaries is mostly
    #  pointless, as we absolutely *MUST* support key collisions in the general
    #  case. For example, consider a callable:
    #      @beartype
    #      def empty_str(text: Literal['']) -> Literal['']: return ''
    #  In this case, those two literal arguments are identical, because the
    #  empty string is a singleton. In turn, this means that parameter names
    #  embedding the object IDs of those literal arguments are also identical,
    #  which means that key collisions are in general absolutely okay. The only
    #  exception to this permissiveness is:
    #  * Neither the "func_params_locals" nor "func_return_locals" dictionaries
    #    may contain a key also contained in the initial local
    #    "func_wrapper_locals" dictionary declared below. This is important!
    #* The existing "is_code_params_needs_random_int" item returned by those
    #  functions should instead by replaced by:
    #  * The "'__beartype_getrandbits': getrandbits" key-value pair
    #    unconditionally listed below should be removed.
    #  * If "is_code_params_needs_random_int" is true, then Those functions
    #    should conditionally add the "'__beartype_getrandbits': getrandbits"
    #    key-value pair to the returned "func_wrapper_locals" dictionary.
    #  * The code testing for "is_code_params_needs_random_int" below should
    #    instead test for:
    #        if '__beartype_getrandbits' in func_wrapper_locals
    ...
    # Dictionary mapping from the name to value of each input wrapper attribute
    # (i.e., attribute accessed in the body of this wrapper). For efficiency,
    # each attribute is defined as an optional private beartype-specific
    # parameter defaulting to that attribute's desired initial value.
    #
    # To avoid colliding with the names of arbitrary caller-defined parameters,
    # the names of these attributes *MUST* be prefixed by "__beartype_".
    func_wrapper_locals = {
        ARG_NAME_FUNC: data.func,
        ARG_NAME_TYPISTRY: bear_typistry,
        #FIXME: Generalize to a similar "ARG_NAME_GETRANDBITS" constant.
        '__beartype_getrandbits': getrandbits,
        #FIXME: Generalize to a similar "ARG_NAME_RAISE_EXCEPTION" constant.
    ...
    # parameters directly derived from the above input wrapper attributes.
    func_wrapper_params = ''.join(
    ...
    code_init = (
        ...
        if (
            is_code_params_needs_random_int or
            is_code_return_needs_random_int
        ) else
