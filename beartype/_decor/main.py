...
#  the current callable parameter or return value is first defined:
#      pep_code_check_param()
#      pep_code_check_return()
#      nonpep_code_check_param()
#      nonpep_code_check_return()
#  It could certainly be worse, but it could certainly be better. *shrug*
#* For each such higher-level non-memoized caller:
...
#FIXME: We'll also need to perform the equivalent for "hint_label" substrings
#currently embedded in *CODE* generated by lower-level memoized callables,
#ideally leveraging the exact same approach as above. The only difference is
#the exception-specific approach above only conditionally formats strings in
#its calls to the reraise_exception_reformatted() function. For the case of
#code, however, we'll want to unconditionally reformat hint labels returned by
#lower-level memoized callables.
#FIXME: Lastly, we'll want to add validation code to the codemain.code()
#function resembling this:
#
#* Unconditionally catch all exceptions and validate that they do *NOT* contain
#  magic substrings before reraising those exceptions with their original
#  tracebacks preserved as is.
#* Unconditionally validate that the "func_code" string localized below also
#  contains *NO* magic substrings.
#
