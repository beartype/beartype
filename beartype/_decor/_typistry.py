...
# ....................{ REGISTRARS ~ tuple                }....................
@callable_cached
def register_typistry_tuple(hint: tuple) -> tuple:
    '''
    ...
    ----------
    hint : type
        Set of all PEP-noncompliant types to be registered.
    ...
    # If this tuple only contains one type, efficiently reduce to registering
    # this type rather than inefficiently registering this tuple.
    if len(hint) == 1:
        return register_typistry_type(hint[0])
    # Else, this tuple either contains no types or two or more types.

    # Return a Python expression evaluating to this tuple by converting this
    # tuple to a set (thus ignoring duplicates) and defer to this lower-level
    # function accepting sets.
    return register_typistry_tuple_from_frozenset(frozenset(hint))


@callable_cached
def register_typistry_tuple_from_frozenset(hint: frozenset) -> tuple:
    '''
    Register the passed frozen set of one or more **PEP-noncompliant types**
    (i.e., classes neither defined by the :mod:`typing` module *nor*
    subclassing such classes) with the beartypistry singleton as a tuple of all
    items in this frozenset *and* return a 2-tuple of a Python expression
    evaluating to this tuple when accessed via the private ``__beartypistry``
    parameter implicitly passed to all wrapper functions generated by the
    :func:`beartype.beartype` decorator *and* the string uniquely identifying
    this tuple as a beartypistry key.
    ...
    tuple factories (e.g., :attr:`beartype.cave.NoneTypeOr`,
    :attr:`typing.Optional`) and thus have no reasonable fully-qualified names.
    Instead, the string uniquely identifying this tuple as a beartypistry key
    ...
    * This tuple's hash. Note that this tuple's object ID is intentionally
    ...
    ----------
    TypeError
        If any item of this frozen set is unhashable and thus *not* a type.
    _BeartypeDecorBeartypistryException
        If this frozen set is either:

        * *Not* a frozen set.
        * Is a frozen set containing one or more items that are either:
    ...
    # If this object is *NOT* a frozenset, raise an exception.
    if not isinstance(hint, frozenset):
        raise _BeartypeDecorBeartypistryException(
            'Beartypistry frozenset {!r} not a frozenset.'.format(hint))
    # Else, this object is a frozenset.

    # If this set only contains one type, efficiently reduce to registering
    # this type rather than inefficiently registering this set.
    if len(hint) == 1:
        return register_typistry_type(hint[0])
    # Else, this frozenset either contains no types or two or more types.

    # Tuple of all types in this set.
    #
    # Note this implicitly raises a "TypeError" exception if any item of this
    # set is unhashable and hence *NOT* a type.
    hint_tuple = tuple(hint)
    ...
    bear_typistry[hint_name] = hint_tuple
