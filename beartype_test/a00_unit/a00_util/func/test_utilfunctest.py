...
def test_is_func_async() -> None:
    ...
    assert is_func_async(coroutine_factory) is True

    # Assert this tester rejects pure-Python coroutine objects.
    assert is_func_async(coroutine) is False
...
def test_is_func_async_coroutine() -> None:
    ...
    # Assert this tester accepts pure-Python coroutine callables.
    assert is_func_async_coroutine(coroutine_factory) is True

    # Assert this tester accepts pure-Python coroutine objects.
    assert is_func_async_coroutine(coroutine) is False
    ...
    # Assert this tester rejects arbitrary non-asynchronous objects.
    assert is_func_async_coroutine('To hear—an old and solemn harmony;') is False
