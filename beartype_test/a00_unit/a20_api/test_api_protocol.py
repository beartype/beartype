...
def test_caching_protocol_as_drop_in_replacement() -> None:
    ...
    if not hasattr(_protocol, "_CachingProtocolMeta"):
        pytest.skip("beartype._protocol._CachingProtocolMeta not available")
    ...
    class SupportsRoundFromScratch(typing.Protocol[_T_co]):
...
def test_protocols_in_annotation_validators() -> None:
    import pytest
    from beartype import typing, _protocol

    if not hasattr(typing, "Annotated"):
        pytest.skip("beartype.typing.Annotated not available")

    if not hasattr(_protocol, "_CachingProtocolMeta"):
        pytest.skip("beartype._protocol._CachingProtocolMeta not available")

    from abc import abstractmethod
    from beartype import beartype, roar, vale
    ...
    def _there_can_be_only_one(
        n: typing.Annotated[  # type: ignore[name-defined]
            SupportsOne, vale.Is[lambda x: x.one() == 1]
        ],
    ) -> int:
        val = n.one()
...
def test_caching_supports_protocols() -> None:
    ...
    _assert_isinstance(int, float, bool, Decimal, Fraction, target_t=typing.SupportsFloat)
    ...
    _assert_isinstance(int, float, bool, Decimal, Fraction, target_t=typing.SupportsRound)
