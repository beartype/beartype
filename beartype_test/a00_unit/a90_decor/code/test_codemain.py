...
def test_codemain() -> None:
    ...
    for hint_meta in HINTS_META:
        ...
        # the same PEP hints.
        for _ in RANGE_2:
            _test_hint_code(hint_meta)

# ....................{ PRIVATE                           }....................
def _test_hint_code(hint_meta: HintPepMetadata) -> None:
    '''
    Test the :func:`beartype.beartype` decorator with respect to type-checking
    code dynamically generated by the :mod:`beartype._decor._code.codemain`
    submodule for the hint described by the passed metadata.

    Parameters
    ----------
    hint_meta : HintPepMetadata
        Metadata describing the hint to be tested.
    '''
    ...
    # Undecorated callable both accepting a single parameter and returning a
    # value annotated by this hint whose implementation trivially reduces to
    ...
    for pith_meta in (
        ...
        pith = (
            # If this pith is actually a pith factory (i.e., callable accepting
            # *NO* parameters and dynamically creating and returning the value
            # to be used as the desired pith), call this factory and localize
            # its return value.
            pith_meta.pith()
        ...
        # Context manager under which to validate this pith against this hint,
        # defined as either...
        pith_context_manager = (
            ...
            if (
                ...
                # This pith should be safely opened and closed as a context
                # rather than preserved as a context manager...
        ...
        with pith_context_manager as pith_context:
            ...
            # Else, this pith satisfies this hint. In this case...
                ...
                # context against this hint *WITHOUT* modifying this context.
